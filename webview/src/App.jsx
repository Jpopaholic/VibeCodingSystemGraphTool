        ...
        <div className="welcome-card">
          ...
          <div className="dream-input-wrapper">
            <button 
              ...
              onClick={async () => {
                const result = await showPrompt(t('promptGoalPrompt'), '例如：做一個結合 IndexedDB 存檔的 Markdown 編輯器，且能匯出 PDF', false, true);
                if (result) {
                  const goal = typeof result === 'string' ? result : result.text;
                  const images = typeof result === 'string' ? [] : result.images;
                  let bootstrapPrompt = language === 'en'
